...
#include "backend_avx2_openmp.hpp"
...
static Args ParseArgs(const int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (k == "--m") { need(i); ParseInt(argv[++i], a.M); }
      ...
      std::cout << "Usage: bench_avx2_openmp [--m M] [--n N] [--k K] [--iters R] "
...
int main(int argc, char** argv) {
  ...
  Auaoalg::AVX2OpenMP::MatrixMultiply(A.data(), B.data(), C_user.data(), M, N, K, num_threads);
  ...
  std::cout << "AVX2+OpenMP   : " << best_user_ms << " ms | " << Gflops(M, N, K, best_user_ms) << " GFLOP/s\n";
  return 0;
}
