...
struct Args {
  ...
  int iters = 10;
  int warmup = 2;
  int threads = 0;  // 0 means use all available
...
static Args ParseArgs(const int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (k == "--m") { need(i); ParseInt(argv[++i], a.M); }
      ...
      std::cout << "Usage: bench_openmp [--m M] [--n N] [--k K] [--iters R] "
...
int main(int argc, char** argv) {
  ...
  Auaoalg::OpenMP::MatrixMultiply(A.data(), B.data(), C_user.data(), M, N, K, num_threads);
  ...
  std::cout << "M=" << M << " N=" << N << " K=" << K << " | threads=" << num_threads << "\n";
  std::cout << "Eigen (MT) : " << best_eigen_ms << " ms | " << Gflops(M, N, K, best_eigen_ms) << " GFLOP/s\n";
  std::cout << "OpenMP     : " << best_user_ms << " ms | " << Gflops(M, N, K, best_user_ms) << " GFLOP/s\n";
  return 0;
}
