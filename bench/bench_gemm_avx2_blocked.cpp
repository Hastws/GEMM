...
#include "backend_avx2_blocked.hpp"
...
static Args ParseArgs(const int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (k == "--m") { need(i); ParseInt(argv[++i], a.M); }
      ...
      std::cout << "Usage: bench_avx2_blocked [--m M] [--n N] [--k K] [--iters R] "
...
int main(int argc, char** argv) {
  ...
  Auaoalg::AVX2Blocked::MatrixMultiply(A.data(), B.data(), C_user.data(), M, N, K);
  ...
  std::cout << "AVX2+Blocked : " << best_user_ms << " ms | " << Gflops(M, N, K, best_user_ms) << " GFLOP/s\n";
  return 0;
}
